...
public class Angle2Controller : MonoBehaviour
{
    ...
    void Update()
    {
        television.SetActive(darkness.activeSelf);
        // 保证是触发视图2跳转
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        worldTouchPositionD.z = darkness.
        ...
        else 
        {
            ...
            {
                // 点击加密盒子外退出特写
                if (box2.activeSelf && darkness.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition))
                {
                ...
                else if (box1.activeSelf && darkness.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    if (!box1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        box1.SetActive(false);
