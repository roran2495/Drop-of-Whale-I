...
public class ComputerDesktopController : MonoBehaviour
{
    ...
    void ChangeFront(bool flag) // false:txt;true:email
    {
        if (!flag)  // txt显示优先级变高，email显示优先级变低
        {
            ...
            emailInput.transform.Find("icon").GetComponent<Canvas>().sortingOrder = 6;
        ...
        else
        {
            ...
            emailInput.transform.GetComponent<SpriteRenderer>().sortingOrder = 8;
            emailInput.transform.Find("icon").GetComponent<Canvas>().sortingOrder = 7;
            emailInput.transform.Find("Canvas").GetComponent<Canvas>().sortingOrder = 7;
            emailContent.transform.GetComponent<SpriteRenderer>().sortingOrder = 6;
        }
    }
