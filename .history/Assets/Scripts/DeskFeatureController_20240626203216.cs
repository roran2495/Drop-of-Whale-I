...
public class DeskFeatureController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
        {
            ...
            if (cup1.activeSelf && cup1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                cupColer.SetActive(true);
                SetColor(new Color(180f/255f, 201f/255f, 174f/255f));
                SetColorFalse();
            ...
            {
                if (!pigmentsFeature.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    pigmentsFeature.SetActive(false);
                ...
                {
                    int i = 0;
                    Transform colorP = pigmentsFeature.transform.Find("colors").transform;
            ...
            else if (eggFeature.activeSelf)
            {
                ...
                {
                    if (eggEgg6.activeSelf || eggEgg7.activeSelf)
                    {
                        Debug.Log("我还是先把现在的这个鸡蛋处理完再加新的蛋吧");
                    }
                    else
                    {
                ...
                else if (!cupLiquid.activeSelf && !cupResin.activeSelf && !cupPigments.activeSelf)
                {
                ...
                {
                    Debug.Log("里面的东西太多了，鸡蛋好难处理");
                }

                if (cupEggs.activeSelf)
                {
                    if (!eggEgg6.activeSelf)
                    {
                        if (!eggEgg7.activeSelf)
                        {
                            flagEggManage = 3;
                        }
                        else
            ...
            // 点击色料选择颜色 色料特写
            else if (pigments.activeSelf && pigments.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                DisableSonColliders(this.transform);
            ...
            else if (cup2.activeSelf)
            {
                if (flagG)
                {
                    if (!cup2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        Debug.Log("已经调制好颜料，可以取走了");
                    }
                    else
                    {
                        GlobalManager.AddItemColor("25", cupColer.transform.Find("color1").GetComponent<SpriteRenderer>().color);
                        cup2.SetActive(false);
                    }
                }
                else
                {
                        ...
                        if (!flagM11)
                        {
                            foreach(Transform child in cupColer.transform)
                            {
                                if (!child.gameObject.activeSelf)
                                {
                                    ...
                                    if (flagM11 || flagM2 || flagM6 || flagM4 || flagM5 || flagW)
                                    {
                                        cupLiquid.transform.position += new Vector3(0,0.3f,0);
                                    }
                                    else
                                    {
                                        cupLiquid.SetActive(true);
                                        cupLiquid.transform.localPosition = new Vector3(cupLiquid.transform.localPosition.x,-0.88f,cupLiquid.transform.localPosition.z);
                                    }
                                    flagM11 = true;
                                    break;
                                }
                    ...
                    else if (materials12.activeSelf && materials12.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        if (!flagM12)
                        {
                            flagM12 = true;
                            cupResin.SetActive(true);
                        }
                        else 
                        {
                            Debug.Log("已经加入，无须反复加入");
                    ...
                    else if (cup2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        ...
                        {
                            item = GlobalManager.FindItem("07");
                            if (item != null && item.isSelected)
                            {
                                if (item.usedTimes == 0)
                                {
                            ...
                            else
                            {
                                ...
                                if (cupEggs.activeSelf)
                                {
                                    ...
                                    eggEgg3.SetActive(true);
                                    eggEgg6.SetActive(flagEggManage >= 2? false : true);
                                    eggEgg7.SetActive(flagEggManage == 1 || flagEggManage == 3 ? false : true);
                                }
                                else
                                {
                                    // 无蛋
                    ...
                    {
                        if (!cupColer.transform.Find("color6").gameObject.activeSelf || !cupPigments.activeSelf)
                        {
                            Debug.Log("还有材料没有加入不用进行搅拌");
                        }
                        else if (eggEgg6.activeSelf || eggEgg7.activeSelf)
                        {
                            Debug.Log("鸡蛋没有处理好");
                        }
                        else
    ...
    public void HandleRGBChoose()
    {
        ...
        {
            float c1 = rgb1.text == "" ? 0 : float.Parse(rgb1.text);
            float c2 = rgb2.text == "" ? 0 : float.Parse(rgb2.text);
            float c3 = rgb3.text == "" ? 0 : float.Parse(rgb3.text);
            GlobalManager.AddItemColor("24", new Color(c1/255f, c2/255f, c3/255f));
        }
    }
    public void HandleRGBChange()
    {
        ...
        float c2 = rgb2.text == "" ? 0 : float.Parse(rgb2.text);
        float c3 = rgb3.text == "" ? 0 : float.Parse(rgb3.text);
        pigmentsFeature.transform.Find("color").GetComponent<SpriteRenderer>().color = new Color(c1/255f, float.Parse(rgb2.text)/255f, float.Parse(rgb3.text)/255f); 
    }
