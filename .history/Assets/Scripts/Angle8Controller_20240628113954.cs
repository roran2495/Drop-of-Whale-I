    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        if (glassWall.activeSelf && !glassWallWet)
        {
            ...
            else if(glassWall.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !glassWall.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                else
                {
                    ...
                    {
                        SetWord(1, "好厚的玻璃墙，应该无法砸开。不过总感觉上面涂了什么，可惜看不出来……或许可以和小说里一样泼水看看, 反正没反应就泼着玩. 老板!我发四我是专业的侦探!如果有什么发现绝对不是瞎猫碰上死老鼠");
                    }
                }
            }
        }
        // 从视角8到视角3
        else if (backArrow && backArrow.activeSelf && backArrow.GetComponent<Collider2D>().bounds.Contains(worldTouchPosition))
        {
            Debug.Log("8->3");
        ...
        else if (!darkness.activeSelf  && !darkness2.activeSelf)
        {
            ...
            if (easel.activeSelf && easel.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                darkness.SetActive(true);
                if (easelFeature.transform.Find("drawing").childCount == 0)
                {
