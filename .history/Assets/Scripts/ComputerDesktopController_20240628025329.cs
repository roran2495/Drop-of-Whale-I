...
public class ComputerDesktopController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
        {
            ...
            // 点击底部txt切换隐藏显示txt
            else if (txtIcon.activeSelf && txtIcon.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                //flagTmp = true说明现在是显示txt
                // 处理显示优先级
                ChangeFront(!flagTmp);
            }
            // 点击底部email切换隐藏显示email
            else if (emailIcon.activeSelf && emailIcon.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                //flagTmp = true说明现在是显示email
                // 处理显示优先级
                ChangeFront(flagTmp);
            }

            // 点击txt打开txt
            else if (desktopIconTxt.activeSelf && desktopIconTxt.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                Debug.Log("1");
                txt.SetActive(true);
            ...
            // 关闭txt
            else if (texttxtInputClose.activeSelf && txtInputClose.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                txtInput.SetActive(false);
                txtIcon.SetActive(false);
            }
            else if (txtContentClose.activeSelf && txtContentClose.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                txtContent.SetActive(false);
                txtIcon.SetActive(false);
            }
            // 处理Icon
            else if (!txtIcon.activeSelf && emailIcon.activeSelf)
            {
            ...
            // txt密码提交
            else if (txtInputSubmit.activeSelf && txtInputSubmit.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                {
                    inputFieldTxt.text = "";
                    SetWord(1, "没有反应，看来是密码错误了，我记得这种加密文件似乎有次数限制，不知道次数到了会怎么样");
                }
            }

            // 点击email打开email
            else if (desktopIconEmail.activeSelf && desktopIconEmail.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
            ...
            // 关闭email
            else if (emailInputClose.activeSelf && emailInputClose.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                emailInput.SetActive(false);
                emailIcon.SetActive(false);
            }
            else if (emailContentClose.activeSelf && emailContentClose.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                emailContent.SetActive(false);
                emailIcon.SetActive(false);
            }
            ...
            // email登录
            else if (emailInputSubmit.activeSelf && emailInputSubmit.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                else
                {
                    ...
                    SetWord(1, "没有反应，看来是账号或者错误了，再仔细检查一下怎么样");
                }
            }
        }
    }
