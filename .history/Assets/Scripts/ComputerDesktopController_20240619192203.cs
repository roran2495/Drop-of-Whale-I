...
public class ComputerDesktopController : MonoBehaviour
{
    ...
    private string inputEmailTextA;
    private string inputEmailTextP;
    ...
    void Start()
    {
        ...
        inputFieldEmailAccount.onEndEdit.AddListener(HandleInputEndEmailAccount);
        inputFieldEmailPassword.onEndEdit.AddListener(HandleInputEndEmailPassword);
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
        {
            ...
            {
                if (inputEmailTextA == "STP93514@myemail.com" && inputEmailTextP == "Nn0514")
                {
                    flagEmail = true;
    ...
    public void HandleInputEndEmailAccount(string txt)
    {
        inputEmailTextA = txt;
        // 当结束输入时调用，显示前部分字符串
        inputFieldEmailAccount.text = finalText.Substring(0, Mathf.Min(finalText.Length, 50)); // 显示最多前50个字符
    }
    public void HandleInputChangeEmailAccount(string newText)
    {
        // 当输入内容变化时调用
        int maxLength = 50; // 设置聊天框的最大长度为50个字符
