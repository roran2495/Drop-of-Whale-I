    ...
    void Update()
    {
        if (flagResolve22)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
            {
                ...
                if (open.activeSelf && open.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("open");
                    F19.transform.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("others/matchbox 2");
                }
                else if (get.activeSelf && get.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("get");
