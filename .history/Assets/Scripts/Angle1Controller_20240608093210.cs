    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                {
                    // 从视角1到视角5
                    if (GlobalManager.FindItem("05").isSelected && doorToAngle5.activeSelf && doorToAngle5.GetComponent<Collider2D>().bounds.Contains(worldTouchPosition))
                    {
                        Debug.Log("5->1");
                ...
                // 点击电话外 ：取消特写
                else if (phone2.activeSelf && !phone2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    darkness.SetActive(flagCurtain && flagSwitcher);
                    phone2.SetActive(!phone2.activeSelf);
                    // 启用其他对象的碰撞器
