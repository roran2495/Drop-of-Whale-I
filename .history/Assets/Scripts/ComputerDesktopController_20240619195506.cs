...
public class ComputerDesktopController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
        {
            ...
            {
                Debug.Log("txtContent.activeSelf:" + txtContent.activeSelf);
                Debug.Log("emailInput.activeSelf:" + emailInput.activeSelf);
                Debug.Log("emailContent.activeSelf:" + emailContent.activeSelf);
            ...
            {
                if (inputTxt.text == "MX07" || inputFieldTxt.text == "mx07")
                {
                    flagTxt = true;
                    txtInput.SetActive(false);
            ...
            {
                if (inputAccount == "STP93514@myemail.com" && inputFieldEmailPassword.text == "Nn0514")
                {
                    flagEmail = true;
    ...
    public void HandleInputEndTxt(string txt)
    {
        inputTxt = txt;
        // 当结束输入时调用，显示前部分字符串
        inputFieldTxt.text = txt.Substring(0, Mathf.Min(txt.Length, 12)); // 显示最多前50个字符
    }
    public void HandleInputChangeTxt(string newText)
    {
        // 当输入内容变化时调用
        int maxLength = 12; // 设置聊天框的最大长度为50个字符

        if (newText.Length > maxLength)
        {
            // 截取后部分字符串显示
            inputFieldTxt.text = newText.Substring(newText.Length - maxLength);
        }
    }
    public void HandleInputEndEmailAccount(string txt)
    {
        inputAccount = txt;
        // 当结束输入时调用，显示前部分字符串
        inputFieldEmailAccount.text = txt.Substring(0, Mathf.Min(txt.Length, 11)); // 显示最多前50个字符
    }
    public void HandleInputChangeEmailAccount(string newText)
    {
        // 当输入内容变化时调用
        int maxLength = 11; // 设置聊天框的最大长度为50个字符

        if (newText.Length > maxLength)
        {
            // 截取后部分字符串显示
            inputFieldEmailAccount.text = newText.Substring(newText.Length - maxLength);
        }
    }
    public void HandleInputEndEmailPassword(string txt)
    {
        inputPassword = txt;
        // 当结束输入时调用，显示前部分字符串
        inputFieldEmailAccount.text = txt.Substring(0, Mathf.Min(txt.Length, 11)); // 显示最多前50个字符
    }
    public void HandleInputChangeEmailPassword(string newText)
    {
        // 当输入内容变化时调用
        int maxLength = 11; // 设置聊天框的最大长度为50个字符
        ...
        {
            // 截取后部分字符串显示
            inputFieldEmailPassword.text = newText.Substring(newText.Length - maxLength);
        }
    }
