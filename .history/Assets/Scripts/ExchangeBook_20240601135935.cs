                    ...
                    if (selected.Count == 3)
                    {
                        ...
                        if (!bigBooks.Contains(selected[2]))
                        {
                            ...
                            if (bigBook != "")
                            {
                                ...
                                if (mappings.TryGetValue(bigBook, out pair) && (mappings.TryGetValue(selected[0], out pa)))
                                {
                                    pair.obj.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("angle8/angle8-9-(" + pair1.value + ")-(" + selected[0] + ")");
                                }
                            }
                            // 小书给拆分的小书位置标记
                        }
