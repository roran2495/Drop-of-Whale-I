...
public class Angle3Controller : MonoBehaviour
{
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        // 从视角3到视角4
        if (doorToAngle4.activeSelf && doorToAngle4.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle4.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
        {
            if (!flagUseKeyLD)
            {
        ...
        else if (!darkness.activeSelf)  //非黑暗状态
        {
            ...
            else if (glassWall.activeSelf && glassWall.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                rightArrow.SetActive(!rightArrow.activeSelf);
                backArrow.SetActive(!backArrow.activeSelf);
                SetWord(1,"我的直觉告诉我, 这面玻璃墙不太对劲……或许可以和小说里一样泼水看看, 反正没反应就泼着玩. 老板!我发四我是专业的侦探!如果有什么发现绝对不是瞎猫碰上死老鼠");
            }
            //点击书，显示特写
