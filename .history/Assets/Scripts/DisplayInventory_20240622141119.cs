...
public class DisplayInventory : MonoBehaviour
{
    ...
    public void ShowObject()
    {
        ...
        foreach (var item in GlobalManager.items)
        {
            ...
            {
                // 实例化物品的 UI 元素
                GameObject newItem = Instantiate(itemPrefab24, content);
                // 设置 UI 元素的 Sprite 图片
            ...
            {
                // 实例化物品的 UI 元素
                GameObject newItem = Instantiate(itemPrefab25, content);
                // 设置 UI 元素的 Sprite 图片
                newItem.transform.Find("color").gameObject.GetComponent<SpriteRenderer>().sprite = item.sprite;
                // 设置 UI 元素的父对象为 Content
                newItem.transform.SetParent(content);
                UpdateItemColor(newItem.transform.GetComponent<SpriteRenderer>(), item.isSelected); 
                // 添加点击事件监听器
