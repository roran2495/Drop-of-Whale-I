    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
        {
            ...
            else if (cup2.activeSelf)
            {
                ...
                || (materials5.activeSelf && materials4.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                || (materials4.activeSelf && materials4.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)))
                {
