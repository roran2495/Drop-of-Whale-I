...
public class Angle3Controller : MonoBehaviour
{
    ...
    private bool flagUseKeyLD;
    private bool flagUseKeyRD;
    private bool flagDoorToB;   // first time : false
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        // 从视角3到视角4
        else if (doorToAngle4.activeSelf && doorToAngle4.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle4.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
        {
            if (!flagUseKeyLD)
            {
        ...
        // 从视角3到视角8
        else if (doorToAngle8.activeSelf && doorToAngle8.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle8.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
        {
            if (!flagUseKeyRD)
            {
                Item item = GlobalManager.FindItem("05");
        ...
        else if (!darkness.activeSelf  && !darkness2.activeSelf)  //非黑暗状态
        {
            if (flowers.activeSelf && flowers.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                SetWord(1, "很好看的花, 但是看上去感觉怪怪的, 是为什么呢");
            }
            // 点击衣服，获取衣服
            if (clothes.activeSelf && clothes.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                GlobalManager.AddItem("06", Resources.Load<Sprite>("angle4/angle4-4-(2)"));
                flagClothes = true;
                clothes.SetActive(!flagClothes);
                SetWord(1,"感觉口袋里有什么东西, 但是作为一个正经侦探, 我怎么能随便翻异性的衣物呢……既然这件衣服掉到了地上, 不然帮她放到洗衣机里吧……额, 应该有洗衣机吧");
            }
            // 点击阳台门 ： 打开阳台
            ...
            {
                Debug.Log("玻璃窗视角");
                Angle8Controller angle8ControllerInstance = GameObject.Find("Angle8").GetComponent<Angle8Controller>();
                angle8ControllerInstance.FlagGlassWall(true); 
            ...
            if (mark.activeSelf && mark.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                if (item != null && item.isSelected)
                {
                    ...
                    if (item != null)
                    {
                       ...
                       SetWord(1, "香薰……感觉是很关键的东西, 而且这个赴约, 也很难让人不在意");
                    }
                    else
                    {
    ...
    {
        if (book0.activeSelf && book0.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
        {
            if (direction)  // true 左->右 15432
            {
                if (book1.activeSelf)
                {
                    book1.SetActive(false);
                ...
                else if (book5.activeSelf)
                {
                    book5.SetActive(false);
                    book4.SetActive(true);
                }
                else if (book4.activeSelf)
                {
                    book4.SetActive(false);
                    book3.SetActive(true);
                }
                else if (book3.activeSelf)
                {
                    book3.SetActive(false);
