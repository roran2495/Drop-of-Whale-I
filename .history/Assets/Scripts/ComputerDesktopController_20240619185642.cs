...
public class ComputerDesktopController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended || Input.GetMouseButtonUp(0))
        {
            ...
            if (desktopIconTxt.activeSelf && desktopIconTxt.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                if (txtInput.activeSelf)
                {
                    inputFieldTxt.Select();
                    inputFieldTxt.ActivateInputField(); // 激活输入字段，确保光标可见
                }
            }
            // 关闭txt
            ...
            if (emailInputSubmit.activeSelf && emailInputSubmit.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                {
                    emailInputAccount.GetComponent<TMP_Text>().text = "";
                    emailInputPassword.GetComponent<TMP_Text>().text = "";
    ...
    void HandleInputEndEmailAccount(string txt)
    {
        inputEmailTextA = txt;
    }
    void HandleInputEndEmailPassword(string txt)
    {
        inputEmailTextP = txt;
    }
