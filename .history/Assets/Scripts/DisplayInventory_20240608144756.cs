...
{
    // 获取最后一个item的索引
    int lastIndex = GlobalManager.items.Count - 1;
    if (lastIndex < 0)
    {
        return; // 如果items为空，则直接返回
    }

    // 获取最新添加的item
    Item newItem = GlobalManager.items[lastIndex];

    // 实例化物品的 UI 元素
    GameObject newItemObject = Instantiate(itemPrefab, content);
    // 设置 UI 元素的 Sprite 图片
    newItemObject.transform.Find("object").gameObject.GetComponent<SpriteRenderer>().sprite = newItem.sprite;
    // 设置 UI 元素的父对象为 Content
    newItemObject.transform.SetParent(content);

    // 添加点击事件监听器
    AddClickHandler(newItemObject, newItem);
}
    ...
    private void HandleClick(Item item, GameObject currentGameObject)
    {
        ...
        if (featuresId.Contains(item.id))   // 特写情况
        {
            ...
            {
                if (preSelectedItem != null && featuresId.Contains(preSelectedItem.id))
                {
        ...
        else if (preSelectedItem != null && featuresId.Contains(preSelectedItem.id))
        {
            ...
            if (item.isSelected) // 选择
            {
                ...
                else if (preSelectedItem.id == "19" && item.id == "22")
                {
                    ...
                    if (Vector2.Distance(F22.transform.position, targetPosition) <= 0.1f)
                    {
                        GlobalManager.RemoveItem(preSelectedItem);
                        GlobalManager.RemoveItem(item);
