        ...
        if (Camera.main.transform.position.x == 60 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                if (!Camera.main.transform.Find("darkness").gameObject.activeSelf)
                {
                    ...
                    {
                        if (flagPlace3 !=0 && flagPlace3 != 3 && place3.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                        {
                            selected.Add("3");
                            Debug.Log("点击3 " + selected.Count);
                        }
                        else if (flagPlace2 !=0 && flagPlace2 != 2 && place2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                        {
                            selected.Add("2");
