...
public class Painter : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetMouseButton(0)) // 如果按下鼠标左键
        {
            ...
            Debug.Log("Drawing at texture coordinate: " + textureCoord);
        }
        else
        {
    ...
    {
        // 限制起始点和结束点在有效的纹理坐标范围内
        startPos = new Vector2(Mathf.Clamp01(startPos.x), Mathf.Clamp01(startPos.y));
        endPos = new Vector2(Mathf.Clamp01(endPos.x), Mathf.Clamp01(endPos.y));
        ...
        Debug.Log("DrawLine from " + startPos + " to " + endPos);
    }
