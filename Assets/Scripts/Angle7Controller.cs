...
public class Angle7Controller : MonoBehaviour
{
    ...
    private bool flagDiaryFirstOpen;
    private bool flagBookFirstOpen;
    ...
    void Start()
    {
        ...
        diaryPage = 0;
        flagDiaryFirstOpen = false;
        flagBookFirstOpen = false;
    }
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        else if (!darkness.activeSelf  && !darkness2.activeSelf)
        {
            // 点击座椅，拆卸 获得座椅脚
            if (seat1.activeSelf && seat1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                if (item != null && item.isSelected)
                {
                    ...
                    SetWord(1, "这样就拿到了座椅脚, 有点重实");
                }
                else
                {
                    if (GlobalManager.someItemIsSelected)
                    {
                        SetWord(1, "虽然不是很确定要不要把它拆开, 但是这个绝对没办法把它拆开");
                    }
                    else
                    {
                        SetWord(1, "看样子这个椅子的身体和脚只是简单得用螺丝连接, 可以用螺丝刀把它们分开");
                    }
                }
            }
            // 点击电脑进入特写
            else if (computer.activeSelf && computer.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                EnableComputerDesktopColliders(computerDesktop.transform);
                if (!flagComputerOpen)
                {
                    SetWord(1, "通电了的, 可以通过显示器开关直接打开电脑, 主机也会同时启动的");
                }
            }
            // 点击日记进入特写
            if (cabinet1Diary.activeSelf && cabinet1Diary.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                if(!flagDiaryUnlock)
                {
                    ...
                    psw1 = psw2 = psw3 = psw4 = 0;
                    if (!flagDiaryFirstOpen)
                    {
                        SetWord(1, "虽然, 但是正经人谁写日记哇.\n而且, 日记的密码一般会用什么? 生日? 纪念日?");
                        flagDiaryFirstOpen = true;
                    }
            ...
            else if (cabinet2.activeSelf && cabinet2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                SetWord(1, "没有写是什么, 不知道记录了什么的本子, 要不要打开呢");
            }
        }
        // 日记本特写
        ...
        else if (bookClose.activeSelf || bookOpen.activeSelf)
        {
            ...
            {
                bookContent.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("angle7/angle7-6-(3)");
                bookClose.SetActive(false);
                bookOpen.SetActive(false);
                EnableOtherColliders();
                darkness.SetActive(false);
            ...
            // 点击内部翻页
            else if (bookClose.activeSelf && bookClose.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                bookClose.SetActive(false);
                bookOpen.SetActive(true);
                if (!flagBookFirstOpen)
                {
                    SetWord(1, "这些看样子都是账号和密码……这个居然不上锁, 这难道不比电视机架值得加个保险");
                    flagBookFirstOpen = true;
                }
            }
            else if (bookOpen.activeSelf && bookContent.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                Sprite sprite1 = Resources.Load<Sprite>("angle7/angle7-6-(3)");
                if (bookContent.GetComponent<SpriteRenderer>().sprite == sprite1)
                {
                    bookContent.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("angle7/angle7-6-(4)");
                }
                else
                {
                    bookContent.GetComponent<SpriteRenderer>().sprite = sprite1;
                    bookOpen.SetActive(false);
                    bookClose.SetActive(true);
                }
            }
        ...
        // 电脑特写
        else if (computerBody.activeSelf)
        {
            // 点击外，取消特写
            ...
            else if (computerButton.activeSelf && computerButton.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                if (!flagComputerOpen)
                {
                    ...
                    SetWord(1, "没有反应? 要不等一会儿看看");
                } else
                {
                    SetWord(1, "没有必要再按一次了吧");
                }
            }
            // 与电脑的交互
    ...
    void EnableComputerDesktopColliders(Transform parent)
    {
        ...
        {
            EnableComputerDesktopColliders(child);
        }
    }
