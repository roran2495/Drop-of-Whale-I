...
public class Angle2Controller : MonoBehaviour
{
    ...
    void Start()
    {
        // 递归地禁用当前对象的所有子对象的 active 属性
        DisableAllChildrenRecursively(transform);
        // 将当前对象的 active 属性设置为 true
    ...
    // 递归地禁用所有子对象的 active 属性
    void DisableAllChildrenRecursively(Transform parent)
    {
        // 遍历当前对象的所有子对象
        foreach (Transform child in parent)
        {
            // 将子对象的 active 属性设置为 false
            child.gameObject.SetActive(false);

            // 递归调用，禁用子对象的所有子对象
            DisableAllChildrenRecursively(child);
        }
    }
}
