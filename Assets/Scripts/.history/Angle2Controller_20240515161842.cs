...
public class Angle2Controller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 判断是否处于黑暗中
        if (Angle1Controller.flagCurtain && Angle2Controller.flagSwitcher)
        {
    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == -30 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击柜子1 ：显示内容
                if (cabinet1.activeSelf && cabinet1.GetComponent<PolygenCollider2D>().bounds.Contains(worldTouchPosition))
                {
