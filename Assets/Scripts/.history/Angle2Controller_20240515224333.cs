...
public class Angle2Controller : MonoBehaviour
{
    ...
    private Vector3 lastTouchPosition;  // 上一帧的触摸位置
    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == -30 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                {
                    // 计算拖动方向
                    Vector3 dragDelta = worldTouchPosition - lastTouchPosition;
                    if (dragDelta.x > 0)
                    {
                        Debug.Log("向右拖动");
                        HandleDrag(worldTouchPosition , true);
                    }
                    else if (dragDelta.x < 0)
                    {
                        Debug.Log("向左拖动");
                        HandleDrag(worldTouchPosition , false);
                    }
    ...
    void HandleDrag(Vector3 worldTouchPosition , bool )
