    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == -30 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击柜子4内容 ：关闭柜子4
                if (cabinet4Content.activeSelf && cabinet4Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle7.GetComponent<BoxColider2D>().bounds.Contains(worldTouchPosition))
                {
