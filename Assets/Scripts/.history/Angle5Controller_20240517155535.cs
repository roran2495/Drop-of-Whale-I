...
public class Angle5Controller : MonoBehaviour
{
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        if (!Camera.main.transform.Find("darkness").gameObject.activeSelf)
        {
            ...
            // 获得鸡蛋
            if (refrigeratorEggs.activeSelf && refrigeratorEggs.)
            // 打开冰箱
            if (refrigerator.activeSelf && refrigerator.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
