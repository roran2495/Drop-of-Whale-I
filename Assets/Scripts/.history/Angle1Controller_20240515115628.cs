    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击抱枕1 ：移动（显示抱枕2）
                if (pillow1 && pillow1.activeSelf && (pillow1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) || letter1.GetComponent<PolygonCollider2D>().bounds.Contains))
                {
