    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == -30 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 从视角2到视角7
                if (doorToAngle7 && doorToAngle7.activeSelf && doorToAngle7.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle7.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition) && (!cabinet4Content.activeSelf || cabinet4Content.GetComponent<PolygonCollider>().bounds.C)
                {
                    Debug.Log("2->7");
