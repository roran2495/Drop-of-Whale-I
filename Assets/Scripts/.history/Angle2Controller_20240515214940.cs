...
public class Angle2Controller : MonoBehaviour
{
    ...
    void PictureFeatureControl()
    {
        Camera.main.transform.Find("darkness").gameObject.SetActive(true);
        DisableOtherColliders(pic)
        foreach (Transform child in picture0.transform)
            {
                // 打印子对象的名称
                Debug.Log("Child Object of picture0: " + child.name);
            }
    }
