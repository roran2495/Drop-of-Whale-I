...
public class Angle3Controller : MonoBehaviour
{
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        // 从视角3到视角8
        eif (doorToAngle8 && doorToAngle8.activeSelf && doorToAngle8.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle8.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
        {
            Debug.Log("3->8");
