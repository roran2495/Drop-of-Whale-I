    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击电话 ： 显示电话，实现拨号
                if (phone1.activeSelf && phone1.GetComponent<PolygonCollider2>().bounds.Contains(worldTouchPosition))
                {
                    darkness.SetActive(true);
                ...
                // 点击电话外 ：取消特写
                if (phone2.activeSelf && !phone2.GetComponent<PolygonCollider2>().bounds.Contains(worldTouchPosition))
                {
                    darkness.SetActive(flagCurtain && flagSwitcher);
                    letter2.SetActive(!phone2.activeSelf);
                    // 启用其他对象的碰撞器
