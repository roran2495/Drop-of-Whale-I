...
public class Angle6Controller : MonoBehaviour
{
    ...
    {
        // 保证是触发视图6跳转
        if (mainCamera.transform.position.x == 31.3 && mainCamera.transform.position.y == 15)
        {
            // 检测触摸输入或鼠标点击
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                {
                    Debug.Log("6->2");
                    mainCamera.transform.position = new Vector3(-28.7, 0, -10);
                ...
                {
                    Debug.Log("6->7");
                    mainCamera.transform.position = new Vector3(61.3, 15, -10);
