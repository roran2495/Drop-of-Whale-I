...
public class Angle2Controller : MonoBehaviour
{
    ...
    Vector3 touchStartPosition; // 存储鼠标按下或开始触摸时的位置
    Vector3 touchEndPosition; // 存储鼠标弹起或结束触摸时的位置
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        else 
        {
            ...
            else if(instructions1.activeSelf && !instructions1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)
                    || !instructions1.activeSelf && instructions0.activeSelf && !instructions0.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                Camera.main.transform.Find("darkness").gameObject.SetActive(false);
    ...
    void HandleDrag(Vector3 worldTouchPosition , bool direction)
    {
        ...
        // 说明书特写
            ...
            || !insinstructions0.activeSelf && instructions0.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
        {
            if (direction)  // true 右->左 054321
            {
                if (instructions0.activeSelf && !instructions1.activeSelf)
                {
                    instructions1.SetActive(true);
                    instructions1.GetComponent<PolygonCollider2D>().enabled = true;
                    instructions5.SetActive(true);
                } 
                else if (instructions5.activeSelf)
                {
                    instructions5.SetActive(false);
                    instructions4.SetActive(true);
                }
                else if (instructions4.activeSelf)
                {
                    instructions4.SetActive(false);
                    instructions3.SetActive(true);
                }
                else if (instructions3.activeSelf)
                {
                    instructions3.SetActive(false);
            ...
            else //false 左->右 012345
            {
                ...
                else if (instructions1.activeSelf)
                {
                    instructions2.SetActive(true);
                }
                else if (instructions2.activeSelf)
