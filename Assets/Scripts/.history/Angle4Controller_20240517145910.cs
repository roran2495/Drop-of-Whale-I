...
public class Angle4Controller : MonoBehaviour
{
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        if (!Camera.main.transform.Find("darkness").gameObject.activeSelf)
        {
            ...
            // 放置牙杯
            if (place.activeSelf && place.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
            ...
            // 放入衣服，出现钥匙
            if (washer.activeSelf && washerOpen.activeSelf && washer.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {

            }
            // 打开洗衣机
            else if (washer.activeSelf && washer.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                washerOpen.SetActive(true);
                if (flagClothes)
                {
                    washerClothes.GetComponent<SpriteRenderer>.sprite = Resources.Load<Sprite>("angle4-4-(2)");
                }
            }
            // 关闭洗衣机
            if (washerOpen.activeSelf && washerOpen.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                if (flagClothes)
                {
                    washerClothes.GetComponent<SpriteRenderer>.sprite = Resources.Load<Sprite>("angle4-4-(3)");
                }
            }
