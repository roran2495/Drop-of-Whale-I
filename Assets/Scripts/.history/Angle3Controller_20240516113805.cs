        ...
        if (!Camera.main.transform.Find("darkness").gameObject.activeSelf)  //非黑暗状态
        {
            ...
            if (OpenBookcaseLeft.activeSelf && OpenBookcaseLeft.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                switch (flagBookcase)
                {
                    case 0 :
                        Debug.Log("打开书架左");
                        bookcase.GetComponent<SpriteRenderer>().sprite = Rescourses.Load<Sprite>("angle3/angle3-2-(2)");
                        flagBookcase = 1;
                        break ;
                    case 1 :
                        Debug.Log("关闭书架左");
                        bookcase.GetComponent<SpriteRenderer>().sprite = Rescourses.Load<Sprite>("angle3/angle3-2-(1)");
                        flagBookcase = 0;
                        break ;
                    default :
                        Debug.Log("关闭书架左，打开书架右");
                        flagBookcase = 2;
                        bookcase.GetComponent<SpriteRenderer>().sprite = Rescourses.Load<Sprite>("angle3/angle3-2-(3)");
                }
            }
            //点击书架右
            if (OpenBookcaseRight.activeSelf && OpenBookcaseRight.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                switch (flagBookcase)
                {
                    case 0 :
                        Debug.Log("打开书架右");
                        bookcase.GetComponent<SpriteRenderer>().sprite = Resourses.Load<Sprite>("angle3/angle3-2-(2)");
                        flagBookcase = 2;
                        break ;
                    case 2 :
                        Debug.Log("关闭书架右");
                        bookcase.GetComponent<SpriteRenderer>().sprite = Resourses.Load<Sprite>("angle3/angle3-2-(0)");
                        flagBookcase = 0;
                        break ;
                    default :
                        Debug.Log("关闭书架右，打开书架左");
                        flagBookcase = 1;
                        bookcase.GetComponent<SpriteRenderer>().sprite = Resourses.Load<Sprite>("angle3/angle3-2-(1)");
                }
            }
            //点击书，显示特写
            //点击书外，关闭特写
