    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击窗帘 ： 不使用or使用
                if (curtain.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) && !leftArrow.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("点击窗帘");
                ...
                {
                    // 窗帘使用+开关打开：黑暗效果
                    darkness.SetActive(flagCurtain && flagSwitcher);
                }
                ...
                // 点击扫把1 ：拆卸（显示扫把2）
                if(broom1.activeSelf && broom1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    broom1.SetActive(!broom1.activeSelf);
                    broom2.SetActive(!broom2.activeSelf);
                }
                // 点击地毯1 ：获取钥匙（显示地毯2）
                if(carpet.activeSelf && carpet.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) && carpet.GetComponent<PolygonCollider2D>().enabled)
                {
                    SpriteRenderer spriteRenderer = carpet.GetComponent<SpriteRenderer>();
                    Sprite newSprite = Resources.Load<Sprite>("angle1/angle1-3-(2)");;
                    spriteRenderer.sprite = newSprite; 
                    carpet.GetComponent<PolygonCollider2D>().enabled = true;
                }
                // 点击信件外 ：取消特写
                if (letter2.activeSelf && !letter2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    darkness.SetActive(flagCurtain && flagSwitcher);
                    letter2.SetActive(!letter2.activeSelf);
                    // 启用其他对象的碰撞器
                    EnableOtherColliders(letter2);
                }
                // 点击电话 ： 显示电话，实现拨号
                if (phone1.activeSelf && phone1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    darkness.SetActive(true);
                    ...
                    // 点击电话外 ：取消特写
                    if (phone2.activeSelf && !phone2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        darkness.SetActive(flagCurtain && flagSwitcher);
                        phone2.SetActive(!phone2.activeSelf);
                        // 启用其他对象的碰撞器
                        EnableOtherColliders(phone2);
                    }
                }
                
                // 点击鞋柜 ：显示鞋柜
                if (shoeCabinet1.activeSelf && shoeCabinet1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) && (broom1.activeSelf && !broom1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) || broom2.activeSelf && broom2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)))
                {
                    darkness.SetActive(true);
                    ...
                    // 点击鞋柜外 ：取消特写
                    if (shoeCabinet2.activeSelf && !shoeCabinet2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        darkness.SetActive(flagCurtain && flagSwitcher);
                        shoeCabinet2.SetActive(!shoeCabinet2.activeSelf);
                        // 启用其他对象的碰撞器
                        EnableOtherColliders(shoeCabinet2);
                    }
                }
