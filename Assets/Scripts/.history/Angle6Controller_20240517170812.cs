...
public class Angle6Controller : MonoBehaviour
{
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        if (!Camera.main.transform.Find("darkness").gameObject.activeSelf)
        {
            ...
            // 打开香薰盒
            if (wardrobeUnopenedBox.activeSelf && wardrobeUnopenedBox.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
            ...
            // 关闭衣柜
            if (wardrobeOpen.activeSelf && wardrobeOpenedBox.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                wardrobeOpen.SetActive(false);
            }
            // 打碎玻璃柜
            else if (!flagNecklace && glassCase.activeSelf && glassCase.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                transform.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("angle6/angle");
            }
        } 
        //特写鲸鱼处理
