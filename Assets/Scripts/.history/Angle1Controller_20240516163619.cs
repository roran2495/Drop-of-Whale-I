        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                if(!darkness.activeSelf)    //非黑暗状态
                {
                    ...
                    // 点击抱枕2 ：移动（显示抱枕1）
                    else if (pillow2.activeSelf && pillow2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                    {
                        pillow1.SetActive(!pillow1.activeSelf);
                        pillow2.SetActive(!pillow2.activeSelf);
                    }
                ...
                else if(flagCurtain && flagSwitcher)
                {
