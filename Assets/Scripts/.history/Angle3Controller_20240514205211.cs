    ...
    void Update()
    {
        ...
        if (mainCamera.transform.position.x == 30 && mainCamera.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 从视角3到视角4
                if (doorToAngle4 && doorToAngle4.activeSelf && doorToAngle4.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle4.GetComponent<CircleCollider2D>.bounds(worldTouchPosition))
                {
