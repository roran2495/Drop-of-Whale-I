    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == -30 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击柜子2急救箱 ：获取道具
                else if (cabinet2FirstAndKit.activeSelf && cabinet2FirstAndKit.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet2FirstAndKit.SetActive(false);
                    flagCabinet2 = true;
                }
                ...
                // 点击柜子4内容 ：关闭柜子4
                else  if (cabinet4Content.activeSelf && cabinet4Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
