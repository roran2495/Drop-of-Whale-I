    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == -30 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                && (!cabinet4Content.activeSelf || cabinet4Content.GetComponent<PolygonCollider>().bounds.Contains(worldTouchPosition))
                && (!cabinet5Content.activeSelf || cabinet5Content.GetComponent<PolygonCollider>().bounds.Contains(worldTouchPosition))
                && (!cabinet6Content.activeSelf || cabinet6Content.GetComponent<PolygonCollider>().bounds.Contains(worldTouchPosition)))
                {
                    Debug.Log("2->7");
                ...
                // 点击柜子4 ：显示内容
                if (cabinet4.activeSelf && cabinet4.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                ...
                // 点击柜子4箱子 : 显示特写加密箱
                if (cabinet4Box.activeSelf && cabinet4Box.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("柜子4加密箱");
                }
                // 点击柜子5 ：显示内容
                ...
                // 点击柜子5内容 ：关闭柜子5
                if (cabinet5Content.activeSelf && cabinet5Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet5Content.SetActive(false);
                    cabinet5Keys.SetActive(false);
                }
                // 点击柜子5钥匙 ： 获取道具
                if (cabinet5Keys.activeSelf && cabinet5Keys.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet5Keys.SetActive(false);
                    flagCabinet5 = true;
                }
                // 点击柜子6 ：显示内容
                if (cabinet6.activeSelf && cabinet6.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet6Content.SetActive(true);
                    cabinet6Instructions.SetActive(true);
                }
                // 点击柜子6内容 ：关闭柜子6
                if (cabinet6Content.activeSelf && cabinet6Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet6Content.SetActive(false);
                    cabinet6Instructions.SetActive(false);
                }
                // 点击柜子6说明书 ：显示特写说明书
                if (cabinet6Instructions.activeSelf && cabinet6Instructions.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("柜子6说明书");
                }
                // 点击柜子7 ：显示内容
                ...
                // 点击柜子7内容 ：关闭柜子7
                if (cabinet7Content.activeSelf && cabinet7Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet7Content.SetActive(true);
                    cabinet7Screwdriver.SetActive(false);
                }
                // 点击柜子7螺丝刀 ：获取道具
                if (cabinet7Screwdriver.activeSelf && cabinet7Screwdriver.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    cabinet7Screwdriver.SetActive(false);
                    flagCabinet7 = true;
                }
