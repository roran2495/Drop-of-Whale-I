    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        if (!Camera.main.transform.Find("darkness").gameObject.activeSelf)
        {
            ...
            else if (computer.activeSelf && computer.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                ...
                computerDesktop.SetActive(false);
                Camera.main.transform.Find("darkness").gameObject.SetActive(true);
                DisableOtherColliders(computerBody);
                // 取消禁用其他的电脑桌面触发器
                computerButton.GetComponent<CirclegonCollider2D>().enabled = true;
            }
            // 点击日记进入特写
            if (cabinetDiary.activeSelf && cabinetDiary.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
            ...
            // 关闭床头柜
            if (cabinetOpen.activeSelf && cabinetOpen.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
                cabinetOpen.SetActive(false);
            }
            // 打开床头柜
            else if (cabinet.activeSelf && cabinet.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
            {
