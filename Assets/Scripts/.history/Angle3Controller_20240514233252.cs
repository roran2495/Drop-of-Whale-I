...
public class Angle3Controller : MonoBehaviour
{
    ...
    {
        // 保证是触发视图3跳转
        if (Mathf.Abs(mainCamera.transform.position.x - 31.3f) < 1 && mainCamera.transform.position.y == 0)
        {
            // 检测触摸输入或鼠标点击
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                {
                    Debug.Log("3->4");
                    mainCamera.transform.position = new Vector3(-28.7f, 15, -10);
                ...
                {
                    Debug.Log("3->8");
                    mainCamera.transform.position = new Vector3(61.3f, 0, -10);
                    leftArrow.SetActive(!leftArrow.activeSelf);
                    rightArrow.SetActive(!rightArrow.activeSelf);
                    backArrow.SetActive(!backArrow.activeSelf);
                }
