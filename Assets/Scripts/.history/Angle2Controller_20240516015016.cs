...
public class Angle2Controller : MonoBehaviour
{
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        else 
        {
            ...
            {
                // 点击加密盒子外退出特写
                if (box2.activeSelf && !box2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)
                    || !box2.activeSelf && box1.activeSelf && !box1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)
                    || !box2.activeSelf && !box1.activeSelf && !box0.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)) 
                {
