    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击扫把1 ：拆卸（显示扫把2）
                if(broom1.activeSelf && broom2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    broom1.SetActive(!broom1.activeSelf);
                    broom2.SetActive(!broom2.activeSelf);
                }
                // 点击地毯1 ：获取钥匙（显示地毯2）
                if(carpet1.activeSelf && carpet2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)
                {
                    carpet1.SetActive(!carpet1.active);
                })
                // 点击信件外 ：取消特写
