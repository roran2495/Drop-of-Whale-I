    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                if (pillow1 && pillow1.activeSelf)
                // 点击抱枕1 ：移动（显示抱枕2）
                if (pillow1 && pillow1.activeSelf && (pillow1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) || letter1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)))
                {
                    pillow1.SetActive(!pillow1.activeSelf);
                    pillow2.SetActive(!pillow2.activeSelf);
                }
                // 点击抱枕2 ：移动（显示抱枕1）
                if (pillow2 && pillow2.activeSelf && pillow2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    pillow1.SetActive(!pillow1.activeSelf);
                    pillow2.SetActive(!pillow2.activeSelf);
                }
                ...
                // 点击地毯1 ：获取钥匙（显示地毯2）
                // 点击信件 ：显示信件
                if (letter1 && letter1.activeSelf && !pillow1.activeSelf && letter1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("/////1");
                    Debug.Log(pillow1.activeSelf);
                    darkness.SetActive(!darkness.activeSelf);
                    letter2.SetActive(!letter2.activeSelf);
                }
                // 点击信件外 ：取消特写
                if (letter2 && letter2.activeSelf && !letter2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    Debug.Log("/////3");
                    Debug.Log(darkness.activeSelf);
                    darkness.SetActive(!darkness.activeSelf);
                    letter2.SetActive(!letter2.activeSelf);
                    Debug.Log("/////2");
                    Debug.Log(darkness.activeSelf);
