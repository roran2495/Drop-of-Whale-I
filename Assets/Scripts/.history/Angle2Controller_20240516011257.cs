            ...
            else if(box0.activeSelf)
            {
                ...
                else if (!box1.activeSelf && box0.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    if (!flagCabinet4)   // need right password
                    {
                        ...
                        else if (password2.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                        {
                            psw2 = (psw2 + 1) % 10;
                            password2.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("others/number-" + psw1);
                        }
                        else if (password3.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                        {
                            psw3 = (psw3 + 1) % 10;
                        }
                        else if (password4.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                        {
                            psw4 = (psw4 + 1) % 10;
                        }
