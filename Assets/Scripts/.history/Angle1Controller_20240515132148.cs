    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击鞋柜 ：显示鞋柜
                if (shoeCabinet1.activeSelf && shoeCabinet1.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) && (broom1.activeSelf && !broom1.GameObject<PolygonCollider2D>().bounds.Contains(worldTouchPosition) || broom2.activeSelf)
                {
                    darkness.SetActive(true);
