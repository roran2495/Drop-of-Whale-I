    ...
    void Update()
    {
        ...
        if (Camera.main.transform.position.x == 0 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                // 点击窗帘 ： 不使用or使用
                if (curtain && curtain.activeSelf && curtain.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition) && !leftArrow.GetComponent<CircleCollider2D>().bounds.Contains)
                {
                    Debug.Log("点击窗帘");
