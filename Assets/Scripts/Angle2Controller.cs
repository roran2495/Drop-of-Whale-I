...
public class Angle2Controller : MonoBehaviour
{
    ...
    private bool flagCabinet3FirstOpen;
    private bool flagCabinet4FirstOpen;
    private bool flagCabinet6FirstOpen;
    ...
    void HandleClick(Vector3 worldTouchPosition)
    {
        ...
        && (!cabinet4Content.activeSelf || !cabinet4Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
        && (!cabinet5Content.activeSelf || !cabinet5Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
        && (!cabinet6Content.activeSelf || !cabinet6Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
        && (!cabinet7Content.activeSelf || !cabinet7Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition)))
        {
            if (!flagUseKeyD)
            {
        ...
        else if(!darkness.activeSelf && !darkness2.activeSelf)  //黑暗状态
        {   
            ...
            {
                if (useKey.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                ...
                else 
                {
                    ...
                    else 
                    {
                        ...
                        else if (cabinet3.activeSelf && cabinet3.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                        {
                            ...
                            if (!flagCabinet3FirstOpen)
                            {
                               SetWord(1, "看样子是一些照片, 说不定有我要找的那张合照"); 
                               flagCabinet3FirstOpen = true;
                            }
                            ...
                            else 
                            {
                                ...
                                else if (cabinet5Content.activeSelf && cabinet5Content.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                                {
                                    ...
                                    if (cabinet5.activeSelf && cabinet5.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                                    {
                                        ...
                                        if (!flagCabinet5)
                                        {
                                            SetWord(1, "好多钥匙!1,2,3,4,4把! 我盲猜这是房间门的钥匙");
                                        }
                                    }
                                    // 点击柜子4 ：显示内容
                                    else if (cabinet4.activeSelf && cabinet4.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                                    {
                                        ...
                                        if (!flagCabinet4FirstOpen)
                                        {
                                            SetWord(1, "一个加密箱……看样子密码是4位的");
                                            flagCabinet4FirstOpen = true;
                                        }
                                    }
                                    // 点击柜子6 ：显示内容
                                    else if (cabinet6.activeSelf && cabinet6.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                                    {
                                        ...
                                        if (!flagCabinet6FirstOpen)
                                        {
                                            SetWord(1, "电视机使用说明书……一般情况下, 这种东西会好好留着么");
                                            flagCabinet6FirstOpen = true;
                                        }
                                    } 
                                    // 从视角2到视角7
                                    else if (doorToAngle7 && doorToAngle7.activeSelf && doorToAngle7.GetComponent<BoxCollider2D>().bounds.Contains(worldTouchPosition) && !doorToAngle7.GetComponent<CircleCollider2D>().bounds.Contains(worldTouchPosition))
    ...
    void SetFlag()
    {
        ...
        flagUseKey = false;
        flagUseKeyD = false;
        flagCabinet3FirstOpen = false;
        flagCabinet4FirstOpen = false;
        flagCabinet6FirstOpen = false;
    }
