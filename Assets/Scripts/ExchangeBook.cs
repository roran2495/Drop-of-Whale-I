    ...
    void Update()
    {
        if (Camera.main.transform.position.x == 60 && Camera.main.transform.position.y == 0)
        {
            ...
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began || Input.GetMouseButtonDown(0))
            {
                ...
                else if ((flagPlace2 == 2 && flagPlace3 == 3 || flagPlace2 == 3 && flagPlace3 == 2) && flagPlace4 == 4 && flagPlace6 == 6 && flagPlace7 == 7 && flagPlace8 == 8 && flagPlace9 == 9 && flagPlace10 == 10
                && temperaBook.activeSelf && !temperaBook.GetComponent<PolygonCollider2D>().bounds.Contains(worldTouchPosition))
                {
                    if (!flagFirst)
                    {
                        flagFirst = true;
                        SetWord(1, "似乎已经还原了书的摆放, 看样子这个字好像是画……画好的画? 未完成的画? ");
                    }
                    else
                    {
                        SetWord(1, "已经没必要移动书的位置了");
                    }
